...
import umc.server.baeksstreetmapserver.user.exception.DuplicatedEmailException;
import umc.server.baeksstreetmapserver.user.exception.DuplicatedLoginIdException;
import umc.server.baeksstreetmapserver.user.exception.DuplicatedNicknameException;
...
import static umc.server.baeksstreetmapserver.common.error.exception.ErrorCode.*;
...
public class UserService {
	...
	public PostUserRes createUser(PostUserReq postUserReq) throws Exception {
		if (loginIdDuplicateCheck(postUserReq.getLoginId())) {
			throw new DuplicatedLoginIdException("로그인 아이디 중복입니다.", LOGIN_ID_DUPLICATION);
		}
		if (nicknameDuplicateCheck(postUserReq.getNickname())) {
			throw new DuplicatedNicknameException("닉네임 중복입니다.", NICKNAME_DUPLICATION);
		}
		if (emailDuplicateCheck(postUserReq.getEmail())) {
			throw new DuplicatedEmailException("이메일 중복입니다.", EMAIL_DUPLICATION);
		}
